...
- 地图 `area_cluster` 的景区/区县语义来自随二进制内置的 `internal/maparea/districts.json` 与 `catalog.json`；当前生成基线覆盖 36 个重点城市的 194 个核心区县，完整清单见 `internal/maparea/README.md`。不得在 `/track-map/view` 请求主链路调用外部逆地理编码。生成区县先加载，`catalog.json` 相同 ID 的人工条目随后覆盖；目录使用 GCJ-02 边界框匹配聚合中心，优先级高者优先、同优先级范围更小者优先。未命中必须保留原网格数量气泡，不能臆造区域名。`area.id` 是稳定目录 ID，不等同于 `cluster_id`。
...
track/create(is_running=false) 或 track upload/update 完成轨迹
  ...
  → Scheduler(track_map_index，默认 TRACK_MAP_INDEX_CRON=@every 10m)

...
func (r *MySQLCompanionRepository) UpdateSession(ctx context.Context, session *models.CompanionSession) error {
	...
	res, err := r.db.ExecContext(ctx,
		`UPDATE companion_sessions
		 SET owner_user_id=?, status=?, join_token=?, join_token_expire_at=?, title=?, track_type=?, locate_addr=?, max_members=?, danmaku_enabled=?, started_at=?, ended_at=?, updated_at=?
		 WHERE session_id=?`,
		session.OwnerUserID,
		session.Status,
		session.JoinToken,
...
func scanCompanionSession(row companionSessionRowScanner) (*models.CompanionSession, error) {
	...
	if err := row.Scan(&item.SessionID, &item.OwnerUserID, &item.Status, &item.JoinToken, &item.JoinTokenExpireAt, &item.Title, &item.TrackType, &item.LocateAddr, &item.MaxMembers, &item.DanmakuEnabled, &item.StartedAt, &endedAt, &item.CreatedAt, &item.UpdatedAt); err != nil {
		return nil, err
	}
	if endedAt.Valid {

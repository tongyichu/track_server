...
func ensureMySQLSchema(ctx context.Context, db *sql.DB) error {
	stmts := []string{
		...
		`CREATE TABLE IF NOT EXISTS track_submissions (
			...
			suitable_months_json VARCHAR(128) NOT NULL,
			surface_types_json TEXT NOT NULL,
			transport_modes_json TEXT NOT NULL,
			transport_description VARCHAR(500) NOT NULL DEFAULT '',
		...
		`CREATE TABLE IF NOT EXISTS track_submission_images (
		...
		`CREATE TABLE IF NOT EXISTS track_submission_events (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,
			submission_id VARCHAR(64) NOT NULL,
		...
		`CREATE TABLE IF NOT EXISTS track_map_index_jobs (
			track_id VARCHAR(64) NOT NULL COMMENT '轨迹ID',
			...
			locked_at DATETIME(6) NULL COMMENT 'worker 抢占时间',
			locked_by VARCHAR(64) NOT NULL DEFAULT '' COMMENT 'worker 标识',
			created_at DATETIME(6) NOT NULL,
			updated_at DATETIME(6) NOT NULL,
			succeeded_at DATETIME(6) NULL COMMENT '成功时间',
			last_failed_at DATETIME(6) NULL COMMENT '最近失败时间',
		...
		`CREATE TABLE IF NOT EXISTS track_geo_indexes (
			track_id VARCHAR(64) NOT NULL COMMENT '轨迹ID',
		...
		`CREATE TABLE IF NOT EXISTS track_route_groups (
		...
		`CREATE TABLE IF NOT EXISTS track_route_group_members (
			group_id VARCHAR(64) NOT NULL COMMENT '路线组ID',
			track_id VARCHAR(64) NOT NULL COMMENT '轨迹ID',
		...
		`CREATE TABLE IF NOT EXISTS track_route_introductions (
			id BIGINT UNSIGNED NOT NULL AUTO_INCREMENT,

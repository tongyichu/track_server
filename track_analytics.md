...
> 当前方案先面向客户端接入第三方或自建分析 SDK；若后续新增服务端埋点接收接口，应同步更新 `docs/api/` 与 `AGENTS.md`。
...
```
App SDK 本地队列
  → 埋点采集入口（第三方 SDK 或自建 /analytics/events）
  → 消息队列 / 日志队列
  → 原始明细层 ODS（对象存储或 ClickHouse 明细表）
...
若使用第三方分析平台，采集入口由 SDK 和平台负责，服务端不需要落库。

若自建采集入口，建议新增独立接口 `POST /api/v1/analytics/events` 批量接收事件，并满足：

- 默认可接收未登录事件，因此不能强依赖业务 JWT；已登录时可带 `user_id` 或业务 token 做身份补充。
- 服务端只做基础校验、限流、脱敏和异步写入，不在请求链路里做复杂聚合。
- 写入失败时返回可重试错误，客户端用同一批 `event_id` 重试。
- 采集接口属于客户端协议变更，新增时必须同步更新 `docs/api/`、`docs/api/route-index.md` 和 `AGENTS.md`。
